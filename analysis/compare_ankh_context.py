...
def run_model(placeholder=0):
    with open('../data/reps/coords_ankh_space_ankh_k.pickle','rb') as f:
    ...
    for layer in tqdm(range(n_layers)):
        ...
        mean_comparison = []
        std_comparison = []
        print(coords_ankh_space[layer][1].shape)
        for idx in prot_types:
            ...
            for i in range(len(prot_labels)):
                if (prot_labels==idx)[i]:
                    ...
                    new_ankh_class.append(coords_ankh_space[layer][i].squeeze()[:-1])
            rand_pcloud = [np.random.rand(len(pcloud),coords_ankh_space[layer][0].shape[1]) for pcloud in new_prot_class]
            Metric_class_rand = MetricSpaceComparison(new_prot_class, rand_pcloud, epsilons)
            Metric_class = MetricSpaceComparison(new_prot_class, new_ankh_class, epsilons)
        ...
        print('Done with model '+model_names[0]+' layer '+str(layer+1))
        ...
        ax.set_title(model_names[0]+' Layer '+str(layer+1))
        fig.savefig('../figures/mean_comparison_'+model_names[0]+'_layer_'+str(layer)+'1.png', dpi=300, bbox_inches='tight')

    fig, axs = plt.subplots(1, len(prot_type_names)+1, figsize=(15, 3), dpi=300)
    ...
    fig.savefig('../figures/mean_comparison_'+model_names[0]+'1.png', dpi=300, bbox_inches='tight')
